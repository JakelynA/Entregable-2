...
class InquilinoController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'email' => 'required|email|max:255',//inquilinos,email,
            'servicios' => 'required|array',
            'servicios.*' => 'exists:servicios,id',
        ]);

        $inquilino = new Inquilino();  //:create($request->only('nombre', 'email'));
        $inquilino -> nombre =$request->input('nombre');
        $inquilino -> correo_electronico =$request->input('correo_electronico');
        $inquilino->save();
